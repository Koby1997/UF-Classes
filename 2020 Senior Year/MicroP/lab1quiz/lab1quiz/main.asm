...
.org 0x9999
IN_TABLE:
.db 0x25, 24, 0xDE, 0x0A, 0b11001010, 0x01, 0

.equ Table_size = 7; label used to calculate size of input table
IN_TABLE_END:
.db 7
...
CHECK_1:
	; check if the CONDITION_1 is met (bit 7 is set); if not, branch to 
	; FAILED_CHECK1
	andi r16, 0b10000000		;if bit is not set, this will = 0
	breq FAILED_CHECK1			;if bit is not set, branch to FAILED_CHECK1

								
	elpm r16, Z+				;if it is here, bit 7 is set, load Z back into r16
...
FAILED_CHECK1:
	elpm r16, Z+				;reload value, changed from andi

	ldi r18, 37
	add r16, r18				;add 37
	ldi r18, 0x40
	cp r16, r18					;compare to 0x40
	breq loop					;==0x40 so do nothing
	brlo loop					;< 0x40 so do nothing
								;if here, it was greater than 0x40, so store
	st Y+, r16
	rjmp loop
