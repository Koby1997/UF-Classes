...
public final class Lexer
{
    ...
    Token lexString() throws ParseException
    {
        ...
        if(!chars.has(0))          //if ends in quote
        ...
        while(!match("\""))     //get everything inside the quotes
        {
            ...
            if(!chars.has(0))
            {
                //throw new plc.interpreter.ParseException("Unterminated String", chars.index);
                return chars.emit(Token.Type.OPERATOR); //unterminated strings are Operators right now
            }
        }
