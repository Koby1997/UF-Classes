 ...
 *
 *  Modified: 15 Jan 2020
 *  Created: 1/15/2020 3:37:44 PM
 *  Author: Dr. Schwartz
...
.equ NUM_CALC = 2000000/1024
.def cnt_r17 = r17		;define another name for r17 to hould our current count
.def sum_r0 = r0			;define another name for r0 to hold our current sum
...
Hello: .dw 0x3744					;Note (in memory window) that this is a little 
									;  endian processor
GoodBye: .db 0x37					;Assembler gives a warning of misalignment since 
...
MAIN:
	...
	;  from program memory. SRAM memory is only 8 bits so we don't normally run into this 
...
; ***NOTE***: You CAN NOT ldi for r0-r15
	clr sum_r0		;Clear our sum register to zero. NEVER make assumptions about 
				;  initial values.
...
LOOP:
	...
	brne LOOP		;if the counter is not equal to zero, branch back 
					;   to LOOP
; End of LOOP
					
; Save the Total
	sts Total, sum_r0	;store the sum variable directly to our 
						;   Total variable

; Demo that assembler can do calculations, NUM_CALC=2000000/1024
	ldi r16, byte1(NUM_CALC)	; byte1 is the same as low
	ldi r17, byte2(NUM_CALC)	; byte2 is the same as high 
								; byte3 is the next higher byte
